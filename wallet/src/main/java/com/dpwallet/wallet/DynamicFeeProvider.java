...
import com.dpwallet.core.util.MonetaryFormat;
...
public class DynamicFeeProvider extends ContentProvider {
        ...
        if (!offline && (lastUpdated == 0 || now - lastUpdated > Constants.RATE_UPDATE_FREQ_MS)) {
            ...
            if (newFeeVal_ != null) {
                if (isFetchingFee) {
                    ...
                    Value newFee_ = getAmount( newFeeVal_ );
                    if (newFee_ != null) {
                        config.setFeeValue(newFee_);
                    }
    ...
    @Nullable
    private Value getAmount(String value_) {
        final String str = value_;
        CoinType type = CoinID.typeFromId( "bitcoin.main" );
        MonetaryFormat format = type.getMonetaryFormat();
        Value amount = null;
