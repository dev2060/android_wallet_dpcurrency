...
public class Configuration {
    ...
    public void setFeeValue(final Value feeValue) {
        ...
        prefs.edit().putString(PREFS_KEY_FEES, feesJson.toString()).apply();
        log.info("we are saving new fee {}", feeValue.toFriendlyString());
    }

    private Value getFeeFromJson(JSONObject feesJson, CoinType type) {
        log.info("selected fee name {} ", type.getName());
        String feeStr = feesJson.optString(type.getId());
        /*if (type.getName()=="Bitcoin") {
            //
            String fee_val = getBtcFee_();
            log.info ("selected fee value3 {}", fee_val);
            return Value.valueOf(type, fee_val);
        }*/
        if (feeStr.isEmpty()) {
            //log.info ("selected fee value1 {}");
            return type.getDefaultFeeValue();
        } else {
            log.info ("selected fee value2 {}", feeStr);
    ...
    public String getBtcFee_() {
        return prefs.getString(PREFS_FEE_BTC_D, "300000");//def value is 300
    }

    public double getBtcFeeDouble_() {
        double res = 300000;
        String data_ = getBtcFee_();
        res = Double.parseDouble( data_ );
        return res;
    }
